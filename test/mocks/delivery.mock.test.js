import { expect } from "chai";
import { connectDbSv, disconnectDbSv } from "../../src/utils/test.utils.js";
import DeliveryMockService from "../../src/mocks/services/delivery.mocks.service.js";

describe("Test unitario sobre Mocks de entregas", function(){
    before(async function(){
        this.timeout(10000)
        await connectDbSv()
    })

    it("Se debe generar un mock de entrega", async function(){
        const delivery = await DeliveryMockService.generateMockDeliveries(1)
        expect(delivery).to.be.an("array").and.to.have.length.greaterThan(0)
    })

    it("[error]: Por numero de entregas invalido", async function(){
        try{
            await DeliveryMockService.generateMockDeliveries(0)
            expect.fail("Se esperaba un error, pero no ocurrio")
        } catch(err){
            expect(err.code).to.equal("INVALID_MOCK_COUNT")
            expect(err.statusCode).to.equal(400)
        }
    })

    after(async function(){
        await disconnectDbSv()
    })
})